...
object PanchangCalculator {
    ...
    fun compute(
        ...
        val jdNoon = julianDay(year, month, day, 12.0 - tz)

        val sunLong = sunLongitude(jdNoon)
        val moonLong = moonLongitude(jdNoon)
        val diff = norm360(moonLong - sunLong)

        val tithiIndex = (floor(diff / 12.0).toInt() + 1).coerceIn(1, 30)
        ...
        val tithiEndJd = findBoundary(
            sunriseJd, { jd -> tithiIndexAt(jd) }, tithiIndex - 1, true
        )
        val nakEndJd = findBoundary(
            sunriseJd, { jd -> nakshatraIndexAt(jd) }, nakIndex, true
        )
        val yogaEndJd = findBoundary(
            sunriseJd, { jd -> yogaIndexAt(jd) }, yogaIndex, true
        )
        val karEndJd = findBoundary(
            sunriseJd, { jd -> karanaIndexAt(jd) }, karanaNum, true
        )
